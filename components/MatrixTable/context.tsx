...
const reducer = (state: MatrixTableState, action: MatrixAction): MatrixTableState => {
  ...
  switch (action.type) {
    case 'SET_MATRIX':
      return { ...state, matrix: action.metadata.resetToEmpty ? emptyMatrix : typeof action.payload !== "undefined" ? action.payload : state.originalMatrix }
    ...
    case 'SET_MATRIX_CELL':
      ...
      return {
        ...
        matrix: {
          ...state.matrix,
          [action.payload.PackagePrice]: {
            ...state.matrix[action.payload.PackagePrice],
            [action.payload.Tier]: action.payload.Value
